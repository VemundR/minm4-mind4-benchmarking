...
def mind4_q(mnr, rea, liste_d4, basisaar, startaar):
    res_dict = {}
    ...
    assert isinstance(mnr.index, pd.PeriodIndex), f'The quarterly dataframe does not have a pd.PeriodIndex.'
    ...
    assert pd.Series(liste_d4).isin(mnr.columns).all(), f'{np.setdiff1d(liste_d4, mnr.columns)} are missing in the quarterly dataframe.'
    ...
    assert pd.Series(liste_d4).isin(rea.columns).all(), f'{np.setdiff1d(liste_d4, rea.columns)} are missing in the quarterly dataframe.'
    ...
    for elem in liste_d4:
        ...
        # The leading value.
        dataqf = mnr[(mnr.index.year == startaar) & (mnr.index.quarter == 1)][elem].values

        # Scaling of the leading value.
        datayf = avstemming1*dataqf

        # The following value.
        dataqe = mnr[(mnr.index.year == basisaar) & (mnr.index.quarter == 4)][elem].values

        # Scaling of the following value.
        dataye = avstemming2*dataqe
        
        dataq=np.hstack((dataqf,dataq_,dataqe)) 
        ...
        try:
            ...
            Y=np.zeros((nq,1),dtype=np.float64) 

        
        res_dict[elem] = Y[1:nq-1].flatten()
