...
def mind4(mnr, rea, liste_d4, basisaar, startaar, freq='M'):
    ...
    if (mnr_of_concern == 0).any().any() is np.True_:
        warnings.warn(f'There are only zeroes in {mnr_of_concern.columns[(mnr_of_concern == 0).any()].to_list()} in the {periodely} dataframe.', UserWarning, stacklevel=2)
    ...
    if not set(mnr_of_concern.index.year.unique()).difference(set(rea_of_concern.index.year)) == set():
        raise TypeError(f"There aren't values in both series for {set(mnr_of_concern.index.year.unique()).difference(set(rea_of_concern.index.year))}.")    
    ...
    if not basisaar < 2050:
        raise TypeError('The final year must be less than 2050. Are you sure you entered it correctly?.')
    ...
    for elem in liste_d4:
        ...
        if freq == 'Q':
            datamf = mnr[(mnr.index.year == startaar) & (mnr.index.quarter == 1)][elem].values
        ...
        # The following value.
        if freq == 'M':
            datame = mnr[(mnr.index.year == basisaar) & (mnr.index.month == freq_)][elem].values
        if freq == 'Q':
            datame = mnr[(mnr.index.year == startaar) & (mnr.index.quarter == freq_)][elem].values
